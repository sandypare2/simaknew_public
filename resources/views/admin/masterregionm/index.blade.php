...
@section('title', 'Master Region | SIMAK')
@section('sub-title', 'Master Region ' )
        ...
        <div class="card">
            ...
            <div class="card-body">            
                <form id="dataForm" name="dataForm" class="form-horizontal">
                    ...
                    <input type="hidden" name="kd_region2" id="kd_region2">
                    @csrf
                    <div class="mb-2">
                        <label class="form-label-custom text-muted fs-13" for="nama_region">Nama Region</label>
                        <div class="input-group input-group-merge">
                            <input type="text" class="form-control form-control-sm" id="nama_region" name="nama_region" placeholder="" aria-label="" aria-describedby="basic-icon-default-fullname2" />
                        </div>
                    </div>
                </form>
            </div>
...
$(function() {
    ...
    $('body').on('click', '.edit_row', function () {
        var id = $(this).data('id');
        $.get("{{ route('masterregionm') }}" +'/' + id, function (data) {                
            $('#modelHeading').html("Edit Region");
            $('#ModalForm').modal('show');
            $('#id').val(data.id);
            $('#kd_region2').val(data.kd_region);
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    ...
                    url: "{{url('api/hapus-masterregionm')}}",
