                ...
                <form id="dataForm" name="dataForm" class="form-horizontal">
                    ...
                    <div class="mb-2">
                        ...
                        <div class="input-group input-group-merge">
                            <input type="text" class="form-control" id="nama_mata_kuliah" name="nama_mata_kuliah" placeholder="" aria-label="" aria-describedby="basic-icon-default-fullname2" />
                        </div>
                    </div>
                </form>
            </div>
    ...
    $(function () {                 
        ...
        var table = $('#tbl_list').DataTable({
            ...
            columns: [
                ...
                {data: 'kd_mata_kuliah',name:'kd_mata_kuliah',width:'60px',className: 'dt-center'},
        ...
        $('.add_row').click(function () {
            ...
            $('#modelHeading').html("Input Mata Kuliah");
            $('#ModalForm').modal('show');
            $('#id').val('').trigger('change');
            $('#kd_mata_kuliah2').val('').trigger('change');
            $('#nama_mata_kuliah').val('').trigger('change');
        });        
        
        $('body').on('click', '.edit_row', function () {
            var id = $(this).data('id');
            $.get("{{ route('matakuliahm') }}" +'/' + id, function (data) {                
                $('#modelHeading').html("Edit Mata Kuliah");
                $('#ModalForm').modal('show');
                $('#id').val(data.id);
                $('#kd_mata_kuliah2').val(data.kd_mata_kuliah);
            ...
            swal({
                ...
                if (willDelete) {
                    $.ajax({
                        ...
                        url: "{{url('api/hapus-matakuliahm')}}",
                        success: function (data) {
                            swal({
                                title: "Result",
                                text: 'Sukses hapus mata kuliah',
                                icon: "info",
                                type: "success",
                                dangerMode: false,
                            })                            
