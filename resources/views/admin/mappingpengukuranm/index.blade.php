...
@section('title', 'Matriks Pengukuran | SIMAK')
@section('sub-title', 'Matriks Pengukuran ' )
                ...
                <div class="table-responsive">
                    <table id="tbl_list" class="table data-table-responsive table-hover align-middle table table-nowrap w-100 custom-table">
                        ...
                        <tr>
                            ...
                            <th colspan="3">Range Sasaran Kinerja</th>
                            <th colspan="3">Range Kompetensi Individu</th>
                            <th rowspan="2">Hasil Pengukuran</th>
                        </tr>
                        <tr>
                            <th>Awal</th>
                            <th>Akhir</th>
        ...
        <div class="card">
            ...
            <div class="card-body">            
                <form id="dataForm" name="dataForm" class="form-horizontal">
                    ...
                    @csrf
                    <div class="mb-2">
                        <label class="form-label-custom text-muted fs-13" for="basic-icon-default-fullname">Nilai Sasaran Kinerja</label>
                        <div class="input-group input-group-merge">
                            <select id="nilai_huruf_kinerja" name="nilai_huruf_kinerja" class="form-control select2" style="width:100%;">
                                <option value="-" selected>-</option>
                                @foreach ($masterkinerjam as $data)
                                    <option value="{{ $data->kode_kinerja }}">{{ $data->kode_kinerja }}</option>                                    
                                @endforeach
                            </select>                            
                        </div>
                    </div>
                    <div class="mb-2">
                        <label class="form-label-custom text-muted fs-13" for="basic-icon-default-fullname">Nilai Kompetensi Individu</label>
                        <div class="input-group input-group-merge">
                            <select id="nilai_huruf_individu" name="nilai_huruf_individu" class="form-control select2" style="width:100%;">
                                <option value="-" selected>-</option>
                                @foreach ($masterindividum as $data)
                                    <option value="{{ $data->kode_individu }}">{{ $data->kode_individu }}</option>                                    
                                @endforeach
                            </select>                            
                        </div>
                    </div>
                    <div class="mb-2">
                        <label class="form-label-custom text-muted fs-13" for="basic-icon-default-fullname">Nilai Pengukuran</label>
                        <div class="input-group input-group-merge">
                            <select id="kode_pengukuran" name="kode_pengukuran" class="form-control select2" style="width:100%;">
                                <option value="-" selected>-</option>
                                @foreach ($masterpengukuranm as $data)
                                    <option value="{{ $data->kode_pengukuran }}">{{ $data->kode_pengukuran." | ".$data->nama_pengukuran }}</option>                                    
                                @endforeach
                            </select>                            
                        </div>
                    </div>
...
$(function() {
    ...
    var table = $('#tbl_list').DataTable({
        ...
        columns: [
            ...
            {data: 'nilai_awal_kinerja',name:'nilai_awal_kinerja',width:'50px',className: 'dt-center'},
            {data: 'nilai_akhir_kinerja',name:'nilai_akhir_kinerja',width:'50px',className: 'dt-center'},
            {data: 'nilai_huruf_kinerja',name:'nilai_huruf_kinerja',width:'50px',className: 'dt-center'},
            {data: 'nilai_awal_individu',name:'nilai_awal_individu',width:'50px',className: 'dt-center'},
            {data: 'nilai_akhir_individu',name:'nilai_akhir_individu',width:'50px',className: 'dt-center'},
            {data: 'nilai_huruf_individu',name:'nilai_huruf_individu',width:'50px',className: 'dt-center'},
            {data: 'nama_pengukuran',name:'nama_pengukuran',width:'100px',className: 'dt-center'},
        ],
        "ordering": false,
        "stateSave": true,
        "scrollX": true,
    ...
    $('.add_row').click(function () {
        ...
        $('#modelHeading').html("Input Matriks Pengukuran");
        $('#ModalForm').modal('show');
        $('#nilai_huruf_kinerja').val('').trigger('change');
        $('#nilai_huruf_individu').val('').trigger('change');
        $("#kode_pengukuran").val('').trigger('change');
    });        
    
    $('body').on('click', '.edit_row', function () {
        var id = $(this).data('id');
        $.get("{{ route('mappingpengukuranm') }}" +'/' + id, function (data) {                
            $('#modelHeading').html("Edit Matriks Pengukuran");
            $('#ModalForm').modal('show');
            $('#id').val(data.id);
            $('#nilai_huruf_kinerja').val(data.nilai_huruf_kinerja).trigger('change');
            $('#nilai_huruf_individu').val(data.nilai_huruf_individu).trigger('change');
            $("#kode_pengukuran").val(data.kode_pengukuran).trigger('change');
        })
    }); 
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    ...
                    url: "{{url('api/hapus-mappingpengukuranm')}}",
    ...
    $('#saveBtn').on("click", function(e) {
        ...
        $.ajax({
            ...
            url: "{{ route('mappingpengukuranm.store') }}",
            type: "POST",
            dataType: 'json',
            success: function (data) { 
                $btn.html($btn.data('orig-html'));
