...
@section('title', 'Master Area/Site | SIMAK')
@section('sub-title', 'Master Area/Site ' )
        ...
        <div class="card">
            ...
            <div class="card-body">            
                <form id="dataForm" name="dataForm" class="form-horizontal">
                    ...
                    <input type="hidden" name="kd_area2" id="kd_area2">
                    @csrf
                    <div class="mb-2">
                        <label class="form-label-custom text-muted fs-13" for="basic-icon-default-fullname">Nama Region</label>
                        <div class="input-group input-group-merge">
                            <select id="kd_region" name="kd_region" class="form-control form-control-sm select2" style="width:100%;">
                                @foreach ($masterregionm as $data)
                                    <option value="{{ $data->kd_region }}">{{ $data->nama_region }}</option>                                    
                                @endforeach
                            </select>                            
                        </div>
                    </div>
                    <div class="mb-2">
                        <label class="form-label-custom text-muted fs-13" for="basic-icon-default-fullname">Kode Area/Site</label>
                        <div class="input-group input-group-merge">
                            <input type="text" class="form-control form-control-sm" id="kode_area" name="kode_area" placeholder="" aria-label="" aria-describedby="basic-icon-default-fullname2" />
                        </div>
                    </div>
                    <div class="mb-2">
                        <label class="form-label-custom text-muted fs-13" for="basic-icon-default-fullname">Nama Area/Site</label>
                        <div class="input-group input-group-merge">
                            <input type="text" class="form-control form-control-sm" id="nama_area" name="nama_area" placeholder="" aria-label="" aria-describedby="basic-icon-default-fullname2" />
                        </div>
                    </div>
                    <div class="mb-2">
                        <label class="form-label-custom text-muted fs-13" for="basic-icon-default-fullname">Jenis KPI</label>
                        <div class="input-group input-group-merge">
                            <select id="jenis_kpi" name="jenis_kpi" class="form-control form-control-sm select2" style="width:100%;">
...
$(function() {
    ...
    var table = $('#tbl_list').DataTable({
        ...
        columns: [
            ...
            {data: 'nama_region',name:'nama_region',width:'300px',className: 'dt-left wrap'},
            {data: 'kode_area',name:'kode_area',width:'100px',className: 'dt-center'},
            {data: 'nama_area',name:'nama_area',width:'300px',className: 'dt-left wrap'},
    ...
    $('.add_row').click(function () {
        ...
        $('#kd_area2').val('').trigger('change');
        $('#dataForm').trigger("reset");
        $('#modelHeading').html("Input Region");
        $('#ModalForm').modal('show');
        $('#kd_region').val('').trigger('change');
        $('#jenis_kpi').val('').trigger('change');
    });        
    
    $('body').on('click', '.edit_row', function () {
        var id = $(this).data('id');
        $.get("{{ route('masteraream') }}" +'/' + id, function (data) {                
            $('#modelHeading').html("Edit Region");
            $('#ModalForm').modal('show');
            $('#id').val(data.id);
            $('#kd_area2').val(data.kd_area);
            $('#kd_region').val(data.kd_region).trigger('change');
            $('#kode_area').val(data.kode_area);
            $('#kd_area').val(data.kd_area).trigger('change');
            $('#nama_area').val(data.nama_area);
            $('#jenis_kpi').val(data.jenis_kpi).trigger('change');
        })
    }); 
