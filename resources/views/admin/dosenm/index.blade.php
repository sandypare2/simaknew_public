        ...
        <div class="card">
            ...
            <div class="card-body">            
                <form id="dataForm" name="dataForm" class="form-horizontal">
                    ...
                    <input type="hidden" name="kd_mata_kuliah2" id="kd_mata_kuliah2">
                    @csrf
                    <div class="mb-2">
                        <label class="form-label" for="basic-icon-default-fullname">Nomor Induk</label>
                        <div class="input-group input-group-merge">
                            <input type="text" class="form-control" id="nip" name="nip" placeholder="" aria-label="" aria-describedby="basic-icon-default-fullname2" />
                        </div>
                    </div>
                    <div class="mb-2">
                        <label class="form-label" for="basic-icon-default-fullname">Nama Dosen</label>
                        <div class="input-group input-group-merge">
                    ...
                    <div class="mb-2">
                        <label class="form-label" for="basic-icon-default-fullname">Mata Kuliah</label>
                        <div class="input-group input-group-merge">
                            <select id="kd_mata_kuliah" name="kd_mata_kuliah" class="form-control select2" multiple>
    ...
    $(function () {                 
        ...
        var table = $('#tbl_list').DataTable({
            ...
            columns: [
                ...
                {data: 'nip',name:'nip',width:'80px',className: 'dt-center'},
                {data: 'nama',name:'nama',width:'200px',className: 'dt-left wrap'},
                {data: 'nama_mata_kuliah',name:'nama_mata_kuliah',width:'200px',className: 'dt-left wrap'},
            ],
            columnDefs: [
            {
        ...
        $('.add_row').click(function () {
            ...
            $('#kd_mata_kuliah2').val('').trigger('change');
            $('#kd_mata_kuliah').val('').trigger('change');
            $('#dataForm').trigger("reset");
            $('#modelHeading').html("Input Master Dosen");
            $('#ModalForm').modal('show');
        });        
        
        $('body').on('click', '.edit_row', function () {
            var id = $(this).data('id');
            $.get("{{ route('dosenm') }}" +'/' + id, function (data) {
                var mata_kuliahnya = data.mata_kuliah.split(',');
                $('#modelHeading').html("Edit Master Dosen");
                $('#ModalForm').modal('show');
                $('#id').val(data.id);
                $('#nip').val(data.nip);
                $('#nama').val(data.nama);
                $('#kd_mata_kuliah2').val(data.mata_kuliah);
                $('#kd_mata_kuliah').val(mata_kuliahnya).trigger('change');;
            })
        }); 
            ...
            swal({
                ...
                if (willDelete) {
                    $.ajax({
                        ...
                        url: "{{url('api/hapus-dosenm')}}",
                        success: function (data) {
                            swal({
                                title: "Result",
                                text: 'Sukses hapus data dosen',
                                icon: "info",
                                type: "success",
                                dangerMode: false,
                            })                            
        ...
        $('#saveBtn').on("click", function(e) {
            ...
            $("#kd_mata_kuliah2").val($('#kd_mata_kuliah').val());
            var datanya = $('#dataForm').serialize();
            $.ajax({
                data: $('#dataForm').serialize(),
                url: "{{ route('dosenm.store') }}",
                type: "POST",
                dataType: 'json',
                success: function (data) { 
