...
use App\Exports\KenaikangrademExport;
...
class KenaikangrademController extends Controller
{
    ...
    {      
        ...
        if ($request->ajax()) { 
            ...
            $searchcari = $request->search; 
            $perintah = "";
            if($kd_areacari!="" && $kd_areacari!="semua"){
                $perintah = " and data_pegawai.kd_area='$kd_areacari'";
            }
            if(!empty($searchcari)){
                $perintah = " and (data_pegawai.nip='$searchcari' or data_pegawai.nama like '%$searchcari%')";
            }
            ...
            foreach($rows1 as $row1){
                // $grade2 = $row1->grade2;
                // $tgl_kenaikan2 = $row1->tgl_kenaikan2;
                $id_master_grade = $row1->id_master_grade;
                if($row1->tgl_kenaikan2!=""){
                    $tanggalnya2 = Carbon::parse($row1->tgl_kenaikan2)->subDay();
                    // $tanggalnya2 = Carbon::now()->format('m');
                    // $tanggalnya = Carbon::createFromFormat('Y-m-d', $tanggalnya2)->subDay()->format('Y-m-d');
                ...
                $datanya = [];
                $datanya['nip'] = $row1->nip;
                $datanya['nama'] = $row1->nama;
                $datanya['jabatan'] = $row1->jabatan;
                $datanya['kd_area'] = $row1->kd_area;
                $datanya['nama_area'] = $row1->nama_area;
                $datanya['id2'] = $row1->id2;
                $datanya['grade2'] = $row1->grade2;
                $datanya['tgl_kenaikan2'] = $row1->tgl_kenaikan2;                
                $datanya['id_master_grade'] = $id_master_grade;

                if($tahun_semester!=""){
                    // $rows2 = DB::table('riwayat_talenta')
                    // ->selectRaw("
                    //     nama_talenta,
                    //     count(*) as jumlah_talenta
                    // ")
                    // ->whereRaw("nip='".$row1->nip."' and concat(tahun,semester)>='$tahun_semester'")
                    // ->groupBy('nama_talenta')
                    // ->orderby('tahun','asc')
                    ...
                    $datanya['data_talenta'] = $a;
                    $datanya['rencana_kenaikan'] = $rencana_kenaikan;
                    $datanya['rencana_tgl_kenaikan'] = $tgl_kenaikan;
                } else {
                    $datanya['data_talenta'] = '';
                    $datanya['rencana_kenaikan'] = '';
                    $datanya['rencana_tgl_kenaikan'] = '';
                }
            ...
            return DataTables::of($data)
                ...
                // ->addColumn('data_talenta', function ($data) {
                //     $a = $data->data_talenta;
                //     return $a;
                // })
                // ->addColumn('rencana_grade','')
                // ->addColumn('rencana_tgl_kenaikan','')
                ->addColumn('aksi', function ($data) {
                    ...
                    // $a .= '<a href="javascript:void(0)" data-id="'.$data->id2.'" title="Hapus Data" class="delete_row"><button type="button" class="btn btn-icon btn-sm btn-danger" style="margin-right:0px;"><span class="ti ti-trash ti-sm"></button></a>';
                    $a .= '</div>';
                    return $a;
                })
                ->rawColumns(['aksi','data_talenta'])
                ->make(true);
        }

        return view('admin.kenaikangradem.index',[
    ...
    {
        return Excel::download(new KenaikangrademExport(
            $request->get('kd_areacarinya')
        ), 'Monitoring Talenta.xlsx');
        // return Excel::download(new Kenaikan($minYear), 'users_by_year.xlsx');
    } 
