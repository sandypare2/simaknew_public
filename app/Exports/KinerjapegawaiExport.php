...
class KinerjapegawaiExport implements FromCollection, WithColumnFormatting, WithHeadings, WithMapping, ShouldAutoSize, WithColumnWidths, WithEvents
{
    use Exportable;
    public function  __construct($tahuncarinya,$nipcarinya)
    {
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class    => function(AfterSheet $event) {
                $event->sheet->getDelegate()->getStyle('A1:BB2')
                ->getFill()
                ...
                $event->sheet->mergeCells('M1:P1');
                $event->sheet->mergeCells('Q1:T1');
                $event->sheet->mergeCells('U1:X1');
                $event->sheet->mergeCells('Y1:AB1');
                $event->sheet->mergeCells('AC1:AC2');
                $event->sheet->mergeCells('AD1:AG1');
                $event->sheet->mergeCells('AH1:AK1');
                $event->sheet->mergeCells('AL1:AO1');
                $event->sheet->mergeCells('AP1:AS1');
                $event->sheet->mergeCells('AT1:AW1');
                $event->sheet->mergeCells('AX1:BA1');
                ...
                for ($row = 3; $row <= $totalRows + 1; $row++) {
                    ...
                    // $cellValue2 = $event->sheet->getCell('B' . $rowNumber)->getValue();
                    // if ($cellValue2!="") {
                    //     $event->sheet->mergeCells('B'.$rowNumber.':B'.($rowNumber+2));
                    // }
                }
                ...
                //     $lastRow = $event->sheet->getDelegate()->getHighestRow();
                //     $cellValue = $event->sheet->getCell('D' . $rowNumber)->getValue();
                //     if ($cellValue=="Target") {
                //         $event->sheet->getStyle('D'.$rowNumber.':BB'.$rowNumber)
                //               ->getFill()
                //               ->setFillType(Fill::FILL_SOLID)
                //               ->getStartColor()
                //               ->setARGB('ffffd9cc');
                ...
                //     $cellValue1 = $event->sheet->getCell('A' . $rowNumber)->getValue();
                //     if ($cellValue1!="") {
                //         $event->sheet->mergeCells('A'.$rowNumber.':A'.($rowNumber+2));
                //     }
                //     $cellValue2 = $event->sheet->getCell('B' . $rowNumber)->getValue();
                //     if ($cellValue2!="") {
                //         $event->sheet->mergeCells('B'.$rowNumber.':B'.($rowNumber+2));
                ...
                // foreach ($this->collection() as $rowIndex => $rowData) {
                //     $rowNumber = $rowIndex + 2; // Adjust for header row if present

                    // Example: Color cell 'B' based on 'Status' value
                    // if ($rowData['keterangan'] === 'Target') {
                    //     $sheet->getStyle('D'.$rowNumber)
                    //           ->getFill()
                    //           ->setFillType(Fill::FILL_SOLID)
                    //           ->getStartColor()
                    //           ->setARGB(Color::COLOR_GREEN); // Use ARGB for solid colors
                    // } elseif ($rowData['Status'] === 'Inactive') {
    ...
    public function collection()
    {
        ...
        ->leftJoin('penilaian_pegawai as c','c.kode_kpi','=','kpi_pegawai.kode_kpi')
